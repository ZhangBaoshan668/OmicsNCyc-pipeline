...
taxonomy = read.csv(opts$input, header=T, row.names=1, sep=",", comment.char="", quote = "")
...
#----3.3 绘图分组 Plotting#----
# 输入矩阵矩阵、元数据和分组列，返回ggplot2对象
p = tax_stackplot(taxonomy, metadata, topN = opts$legend, groupID = opts$group, style = "group", sorted = "abundance")+theme(axis.text.x = element_text(angle = 45, hjust = 1))
if (opts$color == "manual1"){
  p = p + scale_fill_manual(values = colorset1)
} else if (opts$color == "Paired"){
  p = p + scale_fill_brewer(palette = "Paired")
